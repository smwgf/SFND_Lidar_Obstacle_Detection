...
{
    pcl::PointCloud<pcl::PointXYZI>::Ptr filterCloud = pointProcessorI->FilterCloud(inputCloud,0.2f,Eigen::Vector4f(-30,-6,-2,1),Eigen::Vector4f(30,6,3,1));
    ...
    boxEgo.y_max = 1.7;
    boxEgo.z_max = -0.4;
    renderBox(viewer,boxEgo,100,Color(0.5,0,0.5));

    //===== clustering =====
    
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessorI->Clustering(segmentCloud.first, 0.45, 10, 700);
...
void cityBlock(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = processPointClouds->Clustering(segmentCloud.first, 0.3, 10, 1000);
...
void simpleHighway(pcl::visualization::PCLVisualizer::Ptr& viewer)
{
    ...
    //renderPointCloud(viewer,lidar.scan(),"point cloud");
    ProcessPointClouds<pcl::PointXYZ> processPointClouds;
    //===== segmentation =====

    std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, pcl::PointCloud<pcl::PointXYZ>::Ptr> segmentCloud = processPointClouds.SegmentPlane(lidar.scan(),100,0.2);
    //renderPointCloud(viewer,segmentCloud.first,"obstCloud",Color(1,0,0));
    //renderPointCloud(viewer,segmentCloud.second,"planeCloud",Color(0,1,0));
